...
1. **The wedged-deal cluster (S1 + S2 + S13).** No working reject, no timeout, no delivery guarantee — three gaps that compound into the same failure: a deal that stalls forever with balances frozen, recoverable only socially. This is not hypothetical; an underfunded payer produced exactly this state during browser testing (the correct half of the behavior — refusing the debit — worked; the missing half is the reject that should have released the deal). Closing any one of the three materially reduces the griefing surface; S1 (reject) is the highest-leverage.
2. **The settle-proof binding (S3).** A follower cites the lead's settle signature in `seen`, but stores peer settles keyed only by signer and takes the newest one — a settle from an *unrelated* deal unblocks it. The cascade proof the protocol calls load-bearing is currently decorative. Needs a protocol-level binding from predecessor settles to the follower's own records.
3. **Federation is unproven (WORKAROUNDS §4).** Every cross-bank guarantee above rides on an HTTP bank-to-bank path that has never run — all testing is same-deployment via in-process dispatch. A second Deno Deploy app and one cross-deployment settlement test would convert a dozen "should work"s into facts.
