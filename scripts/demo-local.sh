#!/usr/bin/env bash
# N-party demo against a local Deno Deploy bank server.
#
# This script:
#   1. Generates four bank keypairs.
#   2. Starts apps/bank/main.ts on localhost:8000.
#   3. Runs four CLI users through mint → deal → accept → nudge/status.
#   4. Stops the server on exit.
#
# Requires: bun, deno, jq, curl on PATH.
...
# Generate keys for alice, bob, carol, dave.
genkey() { bun run scripts/genkey.ts; }
...
step "0. Generate bank keys"
KEYS=$(mktemp -d)
for name in alice bob carol dave; do
  genkey > "$KEYS/$name.env"
  upper=$(echo "$name" | tr '[:lower:]' '[:upper:]')
  eval "export BANK_${upper}_PRIV_KEY=\"$(grep BANK_PRIV_KEY "$KEYS/$name.env" | cut -d= -f2)\""
done

BASE_URL="${BARTER_BASE_URL:-http://localhost:8000}"
ALICE_BANK="$BASE_URL/alice"
BOB_BANK="$BASE_URL/bob"
CAROL_BANK="$BASE_URL/carol"
DAVE_BANK="$BASE_URL/dave"
...
step "1. Start local Deno Deploy bank server"
deno run --allow-env --allow-net --allow-read --allow-write apps/bank/main.ts &
SERVER_PID=$!
trap 'kill $SERVER_PID 2>/dev/null || true; rm -rf "$KEYS"' EXIT
...
curl -fsS "$BASE_URL/" | jq -c . | sed 's/^/  /'

step "2. Four fresh profiles (A, B, C, D) on four banks"
...
step "5. A initiates the deal — records, subscriptions, lead Tx"
cat > "$DEAL_FILE" <<JSON
{
...
step "6. B, C, D accept their deal tokens (follow-sign their own Txs)"
...
step "7. Banks self-advance: approved → held → settled (no client settle call)"
BARTER_PROFILE=$A cli nudge "$DEAL" >/dev/null   # relay any lost pushes
BARTER_PROFILE=$A cli status "$DEAL" | sed 's/^/  /'

step "8. Post-trade balances (sum per coin = 0)"
