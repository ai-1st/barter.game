...
// TWO Account/Account pairs locally (the issue account, which goes negative,
// and the holding account, which goes positive), and the bank settles the
// pair immediately. Account bodies never leave this machine — the bank only
// sees the two distinct account hashes inside the Account docs.
...
export async function runMint(argv: string[]): Promise<number> {
  ...
  // Two distinct accounts → two accounts: issue (negative) + holding (positive).
