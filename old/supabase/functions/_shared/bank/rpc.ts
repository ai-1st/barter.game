// JSON-RPC 2.0 envelope wrapper for bank Edge Functions.
//
// All RPC requests are POSTs to /rpc on the bank, with a signed envelope:
//
...
//   - verifies the ed25519 signature over canonical JSON (sig field removed)
//   - validates `to` matches THIS bank's pubkey (prevents cross-bank replay)
//   - claims the ULID in the replay window (prevents same-bank replay)
//   - dispatches to the registered handler for `method`
//
// Handlers return either a result value (wrapped into a JSON-RPC success
// response) or throw an RpcError (translated into a JSON-RPC error response).

import { verifyDoc } from "../protocol/crypto.ts";
...
/** Error codes — mirror the design doc's "Methods (v1)" table. */
...
/** Top-level RPC handler. Wire it from the Edge Function entrypoint. */
export async function handleRpc(
  ...
  // Claim the ULID. Atomic in Postgres via unique constraint.
  const fresh = await ctx.db.claimUlid(env.pubkey, env.id, env.to);
  if (!fresh) {
    return jsonRpcError(env.id, new RpcError(RpcErrors.REPLAY, `replay: id ${env.id} already seen from ${env.pubkey}`), 400);
  }
